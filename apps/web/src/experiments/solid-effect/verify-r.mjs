...
//     --format=esm --external:solid-js --external:effect
...
import { createRuntime, runEffect, RuntimeContext } from "./.verify/solid-effect.js"
import { SearchConfigLive, searchPackages } from "./.verify/api.js"
...
const withProvider = await Promise.race([
  run(true),
  sleep(8000).then(() => "timeout"),
])
console.log("with provider   :", JSON.stringify(withProvider))

const withoutProvider = await Promise.race([
  run(false),
  sleep(8000).then(() => "timeout"),
])
