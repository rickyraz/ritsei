...
// works), so `yield*` inside a plain generator delegates the Effect directly
// to our driver loop with full inferred types. Each yielded
...
export const RuntimeContext = createContext<
  ManagedRuntime.ManagedRuntime<any, never> | null
>(
  null,
);
...
function resolveFork(): <A, E>(
  effect: Effect.Effect<A, E, any>,
) => Fiber.Fiber<A, E> {
  const runtime = useContext(RuntimeContext);
  return runtime
    ? (effect) => runtime.runFork(effect)
    : (Effect.runFork as any);
}
...
type SagaStep = Effect.Effect<any, any, any>;
...
export function effectAction<Args extends unknown[], R>(
  ...
  const base = action(function* (...args: Args) {
    ...
    while (!step.done) {
      ...
      inFlight = fiber;
      const exit: Exit.Exit<any, any> = yield Effect.runPromise(
        Fiber.await(fiber),
      );
